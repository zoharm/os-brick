...
class NVMeOFConnector(base.BaseLinuxConnector):
    ...
    def _connect_target_volume(self, target_nqn, vol_uuid, portals):
        try:
            ...
            if not NVMeOFConnector.connect_to_portals(target_nqn, portals):
                LOG.error("No successful connections to: %s", target_nqn)
                raise exception.VolumeDeviceNotFound(device=target_nqn)
        dev_path = NVMeOFConnector.get_nvme_device_path(target_nqn, vol_uuid)
        if not dev_path:
    ...
    @staticmethod
    @utils.retry(exceptions=exception.VolumeDeviceNotFound, retries=5)
